...
-- Wallpaper Changer
hl.bind(mainMod .. " + W", hl.dsp.exec_cmd("~/.local/bin/set-wallpaper.sh"))
--------------------------------
...
-- Workspaces 1 through 4 go to DP-1
for w = 1, 4 do
	hl.workspace_rule({ workspace = tostring(w), monitor = "DP-1", persistent = true })
end

-- Workspaces 5 through 7 go to HDMI-A-1
for w = 5, 7 do
...
-- Kitty blur and opacity window rule
hl.window_rule({
	...
	-- Sets the active and inactive opacity to 80%
	opacity = "0.8 0.8",
})
